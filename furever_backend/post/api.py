...
def post_list_feed(request):
    ...
    user_friends_ids = request.user.friends.all().values_list('id', flat=True)  
    posts = Post.objects.filter(created_by_id__in=user_friends_ids)  
...
def comment_like(request, pk):
    ...
    if not comment.likes.filter(created_by=request.user):
        like = Like.objects.create(created_by=request.user)
        ...
        notification = create_notification(request, 'post_comment_like', post_id=comment.id)

        return JsonResponse({'message': 'Comment liked successfully'})
    else:
        return JsonResponse({'message': 'Comment already liked'})
    
@api_view(['POST'])
def comment_dislike(request, pk):
    comment = Comment.objects.get(pk=pk)

    if not comment.dislikes.filter(created_by=request.user):
        dislike = Like.objects.create(created_by=request.user)
        ...
        notification = create_notification(request, 'post_comment_dislike', post_id=comment.id)

        return JsonResponse({'message': 'Comment disliked successfully'})
    else:
        return JsonResponse({'message': 'Comment already liked'})
